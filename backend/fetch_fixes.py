...
def _repair_json(text: str) -> str:
    ...
    # Strip trailing garbage characters like ] that sometimes appear
...
def _partial_extract(text: str) -> dict:
    ...
    # report (longer analysis text - up to 1000 chars for base model detailed responses)
    m = re.search(r'"report"\s*:\s*"([^"]{10,1000})"', text, re.I | re.DOTALL)
    if m:
        result["report"] = m.group(1)
        print(f"[_partial_extract] report = {m.group(1)[:100]}...")
    
    # description (1 sentence summary)
    m = re.search(r'"description"\s*:\s*"([^"]{10,300})"', text, re.I)
    if m:
        result["description"] = m.group(1)
        print(f"[_partial_extract] description = {m.group(1)}")
...
def normalize_output(raw_text: str, model_label: str = "unknown") -> Dict[str, Any]:
    ...
    # Extract report/description for base model user response
    report = parsed.get("report", "")
    if not isinstance(report, str) or not report.strip():
        report = parsed.get("description", "")
