  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-6 mb-5">
       ...
       {/* Veracity pill — new, was silently dropped before */}
      {data.veracity_assessment && data.veracity_assessment !== "unknown" && (
        <div className="mb-4">
          <p className="text-xs text-gray-500 uppercase tracking-wide">Veracity</p>
          <p className="text-sm font-semibold text-gray-800 mt-1 capitalize">
            {data.veracity_assessment.replace(/_/g, " ")}
          </p>
        </div>
      )}
      <div className="mb-4">
        ...
        <div>
          ...
          <p className="text-lg font-bold">
            ...
            >
              ...
              {metrics.risk_delta})*/}
              {metrics.risk_delta !== null && metrics.risk_delta !== undefined ? `(${metrics.risk_delta > 0 ? "+" : ""}${metrics.risk_delta})`: ""}
            </span>
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-lg font-bold">
           {/*} {(metrics.base_confidence * 100).toFixed(1)}% →{" "}
            ...
            {metrics.base_confidence !== "N/A" && metrics.base_confidence !== null ? `${(metrics.base_confidence * 100).toFixed(1)}%`: "N/A"} →{" "}
            {metrics.lora_confidence !== "N/A" && metrics.lora_confidence !== null ? `${(metrics.lora_confidence * 100).toFixed(1)}%`: "N/A"}
            <span
              className={metrics.confidence_delta > 0 ? "text-emerald-600" : "text-red-600"}
            >
              {/*({metrics.confidence_delta > 0 ? "+" : ""}
              {metrics.confidence_delta.toFixed(1)})*/}
              {metrics.confidence_delta !== null && metrics.confidence_delta !== undefined ? `(${metrics.confidence_delta > 0 ? "+" : ""}${Number(metrics.confidence_delta).toFixed(1)})`: ""}
            </span>
          </p>
        </div>
      </div>
...
// DEBUG helper — logs exactly what the frontend received from the API
  const debugApiResponse = (data) => {
    console.group("[DEBUG] /api/compare response");
    console.log("Full response:", data);
    console.log("base_output:", data.base_output);
    console.log("  risk_score:", data.base_output?.risk_score, "| type:", typeof data.base_output?.risk_score);
    console.log("  confidence:", data.base_output?.confidence);
    console.log("  indicators:", data.base_output?.indicators);
    console.log("lora_output:", data.lora_output);
    console.log("  risk_score:", data.lora_output?.risk_score, "| type:", typeof data.lora_output?.risk_score);
    console.log("  confidence:", data.lora_output?.confidence);
    console.log("  indicators:", data.lora_output?.indicators);
    console.log("comparison_metrics:", data.comparison_metrics);
    console.groupEnd();
  };
  ...
  const analyze = async () => {
    ...
    try {
      ...
      debugApiResponse(data);   // <-- prints full breakdown in browser console
    ...
    <div className="bg-[#F7F9FC] min-h-screen text-gray-900">
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-600 flex items-center gap-2">
                    ...
                    {compareMode ? "Compare Base vs LoRA" : "Base model only"}
      ...
      <div className="max-w-6xl mx-auto px-10 pb-16 grid gap-8">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className={`${compareMode ? "lg:col-span-2" : "lg:col-span-3"} grid ${compareMode ? "lg:grid-cols-2" : "lg:grid-cols-1"} gap-6`}>
            <ResultCard
              title="Base Model"
